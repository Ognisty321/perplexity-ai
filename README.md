...
# model = model for mode (own account only). See "Available Models" below.
...
class Client:
    ...
    def search(
        ...
        Args:
            ...
            search_focus: Search focus (e.g., "internet")
            search_recency_filter: Optional recency filter
            prompt_source: Prompt source tag (defaults to "user")
            query_source: Query source tag (defaults to "home")
            supported_block_use_cases: List of supported block use cases
            supported_features: List of supported feature flags
            time_from_first_type: Client timing hint (float)
            dsl_query: Optional DSL query string
            params_overrides: Dict of raw params to override in payload
            wait_for_attachment_processing: Wait for attachment processing SSE before query
            attachment_processing_timeout: Optional timeout override for attachment processing SSE
...
You can pass either the raw model code (recommended) or a friendly alias.
...
```python
{
    ...
    'pro': [
        None,
        'pplx_pro', 'pplx_pro_upgraded',
        'experimental',  # Sonar
        'gpt52', 'gpt51', 'gpt5', 'gpt41',
        'claude45sonnet',
        'gemini30flash',
        'grok41nonreasoning', 'grok4nonthinking',
    ],
    'reasoning': [
        None,
        'pplx_reasoning', 'pplx_study',
        'gpt52_thinking', 'gpt51_thinking', 'gpt5_thinking', 'gpt5_pro',
        'o3pro',
        'claude45sonnetthinking', 'claude45opus', 'claude45opusthinking',
        'claude41opus', 'claude41opusthinking',
        'gemini30pro', 'gemini25pro', 'gemini30flash_high',
        'kimik2thinking',
        'grok41reasoning', 'grok4',
    ],
    'deep research': [
        None,
        'pplx_alpha',
        'claude40sonnetthinking_research', 'claude40opusthinking_research',
        'o3pro_research',
    ],
}
```
...
File uploads are sent via the batch upload endpoint and then referenced as attachment URLs in
the query payload. If you want to mirror the browser flow, enable
`wait_for_attachment_processing` to subscribe to the attachment processing SSE before the query.
...
with Client(cookies=COOKIES) as client:
    result = client.search(
        "Summarize the attached subtitles.",
        mode="pro",
        files={"episode.ass": open("episode.ass", "rb").read()},
        wait_for_attachment_processing=True,
    )
    print(result.get("answer"))
```

You can also run the integration upload test script locally using environment variables:

```bash
cp .env.example .env
# Fill PPLX_CSRF_TOKEN and PPLX_SESSION_TOKEN in .env
python3 scripts/integration_upload_test.py
```
...
```bash
python3 -m pytest tests/ --cov=perplexity --cov=perplexity_async --cov-report=html
```
