...
class Client(AsyncMixin):
    ...
    async def _init_session(self) -> None:
        try:
            resp = await self.session.get(ENDPOINT_AUTH_SESSION, timeout=self._timeout)
    ...
    async def _close_response(self, resp) -> None:
        closer = getattr(resp, "aclose", None)
        if callable(closer):
            await closer()
        else:
            resp.close()
        ...
        *,
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                await self._request("GET", match.group(1))
                self.own = True
    ...
    async def _upload_files(self, files: Mapping[str, bytes]) -> Tuple[List[str], List[str]]:
        ...
        try:
            upload_info_resp = await self._request(
                "POST",
                ENDPOINT_UPLOAD_URL,
        ...
        for file_uuid in file_order:
            ...
            upload_resp = await self._request("POST", bucket_url, multipart=mp)
    ...
    async def _subscribe_attachment_processing(
        self,
        file_uuids: List[str],
        *,
    ...
    async def _iter_sse_messages(self, resp):
    ...
    async def _stream_response(self, resp):
        try:
            async for payload in self._iter_sse_messages(resp):
                yield payload
        finally:
            await self._close_response(resp)

    async def _collect_response(self, resp) -> dict:
        try:
            last_payload: dict = {}
            async for payload in self._iter_sse_messages(resp):
    ...
    async def search(
        ...
        if normalized_files:
            uploaded_files, uploaded_file_uuids = await self._upload_files(normalized_files)
            if wait_for_attachment_processing:
                await self._subscribe_attachment_processing(
                    uploaded_file_uuids,
        ...
        resp = await self._request("POST", ENDPOINT_SSE_ASK, json=json_data, stream=True)
